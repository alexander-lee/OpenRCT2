 ...
 *        thrill Corkscrew Ascent (§4.0-C) + Foundry Flyer (§4.0-B) +
 *        MotionSimulator · transport Grand Circle Monorail + MagneticRide ·
 *        dark HauntedMansion · water RiverRapids + MoonlitBarge ·
 *        gentle Helicycles.
 *        FOUR of the nine are catalog circuits that had NEVER SHIPPED
 *        (MotionSimulator, HauntedMansion, MagneticRide, Helicycles)
 ...
 * FLAG   §4.0-C INVERTING rect at start [16.8, 0.55, -3.6], heading 0, steel,
...
const KEEPOUT: Rect[] = (() => {
  ...
  return [
    // §4.0-C offsets
    { who: 'C-west', x: [cx - 36.0, cx - 33.6], z: [cz - 1.2, cz + 14.4] },
    { who: 'C-south', x: [cx - 27.6, cx - 12.0], z: [cz - 15.6, cz - 13.2] },
    { who: 'C-north', x: [cx - 25.2, cx - 9.6], z: [cz + 21.6, cz + 22.8] },
    { who: 'C-station', x: [cx - 1.2, cx + 1.2], z: [cz - 7.2, cz + 8.4], own: true },
    // §4.0-B offsets
...
const EDGES: [NetRef, NetRef][] = [
  ...
  [18, 22], [22, 23], [23, 24],                      // the WEST spoke foot + W tail
...
const PAD_MARGIN: Record<string, number> = {
  ...
  MotionSimulator: 3.2,    //  2.2 x 2.2 base plate
};
...
const PULSE = worldPlan({
  ...
  rides: [{ at: SIM.pad, name: 'Pulse Simulator' }],
...
const FOUNDRY = worldPlan({
  ...
  rides: [{ at: MANSION.pad, name: 'Foundry Mansion' }],
...
const RIDE_NAMES = [
  'Corkscrew Ascent', 'Foundry Flyer', 'Grand Circle Monorail', 'Pulse Simulator',
  'Foundry Mansion', 'Toadstool Barge', 'Maglev Glider', 'Meadow Helicycles', 'Hollow Rapids',
];
...
export function ThemePark() {
  return (
    ...
    >
      ...
      <MagneticRide position={MAGLEV.pad} rotation={MAGLEV.yaw}
        register={{ name: 'Maglev Glider', capacity: 8, rideDuration: 11, intensity: 2, price: 3 }}
        queue={{ anchor: MAGLEV.anchor, dir: MAGLEV.dir }} />
      <Helicycles position={HELI.pad} rotation={HELI.yaw}
        register={{ name: 'Meadow Helicycles', capacity: 2, rideDuration: 8, intensity: 2, price: 2 }}
      ...
      <MotionSimulator position={SIM.pad} rotation={SIM.yaw}
        register={{ name: 'Pulse Simulator', capacity: 4, rideDuration: 7, intensity: 6, price: 3 }}
      ...
      <HauntedMansion position={MANSION.pad} rotation={MANSION.yaw}
        register={{ name: 'Foundry Mansion', capacity: 6, rideDuration: 11, intensity: 5, price: 4 }}
        queue={{ anchor: MANSION.anchor, dir: MANSION.dir }} />
      <GiantGear position={dry(WORKS_SC[0])} seed={3} />
      <SteamPipes position={dry(WORKS_SC[1])} seed={5} />
      <ClockTower position={dry(WORKS_SC[2])} seed={7} />
...
export function __netdump() {
  const rides = [
    ...
    { name: 'Foundry Flyer', at: [B_START[0], B_START[2]] as XZ, registered: true, centre: FLAG2_CENTRE },
    { name: 'Grand Circle Monorail', at: [-42.6, -9.7] as XZ, registered: true },
    { name: 'Pulse Simulator', at: SIM.pad, registered: true },
    { name: 'Foundry Mansion', at: MANSION.pad, registered: true },
    { name: 'Toadstool Barge', at: BARGE.pad, registered: true },
    { name: 'Maglev Glider', at: MAGLEV.pad, registered: true },
    { name: 'Meadow Helicycles', at: HELI.pad, registered: true },
