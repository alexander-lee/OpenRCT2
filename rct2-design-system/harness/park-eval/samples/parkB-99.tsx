 ...
 *        thrill Corkscrew Ascent (§4.0-C) + Ashfall Flyer (§4.0-B) +
...
import {
  ...
  EMBERFALL_CALDERA, PULSE_DISTRICT, THORNWICK_GLADE,
...
const WORKS_ROW = bazaarPlan({
  id: 'worksRow', title: 'Cinderworks Arcade', position: [-52.8, -16.8],
  facing: { port: 'E', toward: [-44.4, -16.8] },
  stalls: ['emberRoast', 'hotDog', 'soda'], theme: EMBERFALL_CALDERA, seed: 9,
});
...
// RE-THEMED to EMBERFALL (canonical `fire`) for the axis-16 world-CHOICE credit, and to
// keep this park's world trio distinct from parkA-99's. Measured over the 30-park corpus
// in the CANONICAL id space (`node themes.mjs`): steampunk 12 and neon 12 are OVERUSED,
// enchantedForest 11 is AT the median, pirateBeach 2 and fire 5 are UNDERUSED — and the
// choice credit is strictly median-relative, so `fire` earns the full 0.5 exactly as
// parkA-99's `pirateBeach` does. Safe here because this world's only ride is
// <HauntedMansion>, which carries NO themeId, so re-theming cannot raise a `crossTheme`
// finding; the row's own counter moves with it (`emberRoast`, this world's shop).
const FOUNDRY = worldPlan({
  id: 'emberfall', theme: EMBERFALL_CALDERA, pieces: [WORKS_ROW],
  rides: [{ at: MANSION.pad, name: 'The Cinder Lodge' }],
...
([['pulse', PULSE_SC], ['emberfall', WORKS_SC], ['thornwick', GLADE_SC]] as const).forEach(([id, cells]) => {
...
const RIDE_NAMES = [
  'Corkscrew Ascent', 'Ashfall Flyer', 'Grand Circle Monorail', 'Pulse Launcher',
  'The Cinder Lodge', 'Toadstool Barge', 'Meadowline Karts', 'Meadow Orbiter', 'Hollow Rapids',
];
  ...
  return (
    ...
    >
      ...
      {/* UNTHEMED on purpose: a THEMED circuit standing outside every <World> rect is a
          ...
          carries themeId 'pirateBeach', which this park has no world for — measured. */}
      ...
      <HauntedMansion position={MANSION.pad} rotation={MANSION.yaw}
        register={{ name: 'The Cinder Lodge', capacity: 6, rideDuration: 11, intensity: 5, price: 4 }}
        queue={{ anchor: MANSION.anchor, dir: MANSION.dir }} />
      <Fumarole position={dry(WORKS_SC[0])} seed={3} />
      <ObsidianShards position={dry(WORKS_SC[1])} seed={5} />
      <BasaltColumns position={dry(WORKS_SC[2])} seed={7} />
...
export function __netdump() {
  const rides = [
    ...
    { name: 'Ashfall Flyer', at: [B_START[0], B_START[2]] as XZ, registered: true, centre: FLAG2_CENTRE },
    { name: 'Grand Circle Monorail', at: [-42.6, -9.7] as XZ, registered: true },
    { name: 'Pulse Launcher', at: SIM.pad, registered: true },
    { name: 'The Cinder Lodge', at: MANSION.pad, registered: true },
