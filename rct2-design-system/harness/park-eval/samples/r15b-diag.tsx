// DIAGNOSTIC COPY of samples/r15b.tsx — NOT the scored artifact.
// r15b.tsx itself FAILS preflight (no leading §0 PRE-FLIGHT header, no <Park roster>)
// and therefore never bundles. This copy adds ONLY those two metadata items so the
// park can be mounted and probed; NO geometry, coordinate, seed or prop that affects
// the scene has been altered.
//
// §0 PRE-FLIGHT (reconstructed from the shipped file, for gate-satisfaction only)
//   SEED 4 · SIZE 48 · CLIMATE temperate · GATE [0, 22.8]
//   WORLDS: none declared (0)
//   MONORAIL: none (size 48 < 64, so preflight's ring mandate does not fire)
//   ROSTER: rides 6 (Cinder Racer coaster + FerrisWheel, Carousel, Teacups,
//           DropTower, SwingRide) · stalls 4 (Sunset Market bazaar) · categories 2
//   LATTICE: 10 authored nodes + 2 set-pieces (hub FountainPlaza, market Bazaar),
//           11 edges, all cardinal; coaster queue tail [-3.6, -7.2] is a leaf
//   COASTER: 1 (legacy steel rectangle), 1 tracked circuit, 0 inversions
...
export function ThemePark() {
  return (
    <Park
      ...
      size={SIZE}
      background="#a9cfe0"
      roster={{ rides: 6, stalls: 4, categories: 2 }}
