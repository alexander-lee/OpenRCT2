...
| `probe-no-breakdown.mjs` | **CAN A RIDE STILL BREAK DOWN? (2026-07-28)** The gate on the rule that no ride in this design system may EVER break down — a deliberate divergence from RCT2, which does (`GameManager/Context.md` "NO RIDE CAN BREAK DOWN"). No browser: the real `createGameManager` on the `probe-board-quiet.mjs` flat graph and the `probe-monorail-transfer.mjs` 4-platform graph, 1800 sim-s per scenario at dt = 1/30, **`handle.status()` sampled EVERY step** (an 18 s outage is invisible to the browser probe, which samples the sim once ~1.5 s in). Three rides: one registered normally, and TWO that pass `breakdownEvery` — the knob a park could once use to buy a breakdown, which must now buy nothing AND raise exactly one `IGNORED` warning, because a knob that still worked would contradict the rule and one dropped silently would leave an author guessing. Exit 1 on any `brokenDown`/`beingRepaired` step, any `status()` outside open/closed, any ride that served nobody (a scenario that serves no one proves nothing by staying up), or a missing/duplicated IGNORED warning. **`--breakdowns` restores the removed schedule IN MEMORY at bundle time** — esbuild `onLoad`, seven asserted anchors across `rideFsm`/`access`/`needs`/`navigation`/`arrivals`, nothing under `mp3d/` written — and INVERTS the assertion, so that half must OBSERVE downtime: a probe that passes whatever the code does proves nothing, and it is also where the throughput delta comes from (measured: 14.00 % / 34.24 % of the clock out of service → **0.00 %**; the opted-in flat ride 0.03 → 3.40 riders/sim-min, the monorail 0.47 → 2.43 and 14 → 73 transfers; the no-knob flat ride a wash at 3.50 → 3.40). `--secs=` `--guests=` `--cap=` `--dur=` `--every=` `--repo=<mp3d copy>` `--json` |
...
node probe-no-breakdown.mjs [--breakdowns]     # NO RIDE MAY BREAK DOWN — 1800 sim-s, exit 1 on any downtime (A/B)
