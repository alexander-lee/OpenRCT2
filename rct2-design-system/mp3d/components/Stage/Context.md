...
- **THE DAY/NIGHT BUTTON IS TIMED IN SECONDS (2026-07-28).** Reported as "when i turn off lighting there's always a super lag with large park", and it was worse than lag: on `parkA-99` (size 128, **168 lit lamps**, real M4 Pro, `harness/mp3d-render/probe-night-toggle.mjs`) pressing DAY meant the park **never came back** — 60 s after the click `nightK` was still 0.050, all 168 lights still visible, still **1318 ms/frame**, and every later toggle the same. Three causes, all the same shape (a per-FRAME constant standing in for a per-SECOND one): the cross-fade was `nightK += (want − nightK) * 0.06` per frame, so its duration was "however many frames", and a night park's frame is ~80x a day park's — plus the exponential only ever APPROACHED zero, so the night-gated lamps never crossed the cull's threshold and were never shed; `darkCull.tick` was fed the loop's CLAMPED dt, making its 0.25 s cadence three frames long; and the cull let the fade's tail out one lamp-batch at a time, so each toggle invented light counts nothing had compiled. Now: the fade is linear on the UNCLAMPED dt over `NIGHT_FADE_S = 0.8`, so it LANDS exactly on 0/1 in bounded wall clock; the cull gets `dtRaw`; and the tail is coalesced (see `darkLights.ts`). **Measured, same park, three toggle cycles each:**

  | | press NIGHT | press DAY | after |
  |---|---|---|---|
  | before | fade never completes | **never recovers** (>60 s, 168 lights, 1318 ms/frame) | stuck slow forever |
  | after | 4.2 / 5.5 / 5.4 s, ONE light-count change | **6.1 / 7.2 / 7.4 s** | 2 lights, 16-23 ms/frame |

  The picture is unchanged (day screenshots before/after are identical). What is LEFT is not the toggle: a park carrying 168 real point lights costs ~1.3 s/frame at night whatever the button does, and a light count nothing has drawn before costs ~11 shader compiles (13 ms each at 2 lights, seconds each at 60+). **The ≤ 4-real-lights-per-component / ~8-per-scene rule is what actually buys a fast night** — these parks are 20x over it.
