...
export function buildBumperCarsScene(three: typeof THREE, opts: BumperCarsOpts = {}): BumperCarsBuilt {
  ...
  for (let i = -2; i <= 2; i++) g.add(box(t, [A, 0.02, 0.03], 0x33343a, [0, 1.915, (i * A) / 5], { metal: 0.5 })); // slats overlap the panel underside (no air gap)
  ...
  for (let i = 0; i < N; i++) {
    ...
    const skirt = cyl(t, 0.36, 0.4, 0.16, skirts[i], [0, 0.1, 0], { rough: 0.55, seg: 20, emissive: skirts[i] }); // round bumper skirt (emissive flashes on contact)
    ...
    car.add(skirt);
    car.add(cyl(t, 0.41, 0.41, 0.05, 0x111116, [0, 0.04, 0], { rough: 0.7, seg: 20 })); // rubber ring
    car.add(box(t, [0.36, 0.24, 0.46], BODY, [0, 0.28, -0.02], { tex: 'plastic', repeat: [2, 1], rough: 0.45 })); // teal body
    car.add(box(t, [0.38, 0.05, 0.48], skirts[i], [0, 0.18, -0.02], { rough: 0.5 })); // trim line
    car.add(box(t, [0.28, 0.02, 0.2], 0x3c5a5a, [0, 0.405, -0.1], { tex: 'fabric', repeat: [2, 1], rough: 0.95 })); // padded seat cushion on the body top
    car.add(box(t, [0.3, 0.14, 0.04], 0x3c5a5a, [0, 0.45, -0.23], { tex: 'fabric', repeat: [2, 1], rough: 0.95 })); // low backrest cushion (body rear face -0.25)
    car.add(box(t, [0.34, 0.12, 0.14], 0x111116, [0, 0.36, 0.13], { rough: 0.7 })); // dash, fully backed by the body below (no floating overhang)
    car.add(cyl(t, 0.05, 0.05, 0.02, 0x2b2b30, [0, 0.45, 0.1], { rotX: -0.9, metal: 0.4, rough: 0.5, seg: 12 })); // steering wheel at the hands
    car.add(cyl(t, 0.02, 0.02, 1.5, 0x777, [0, 1.0, -0.15], { metal: 0.6, rough: 0.4, seg: 6 })); // pole
    const contact = ball(t, 0.05, 0x66e0ff, [0, 1.75, -0.15], { emissive: 0x2288aa }); // contact brushing the ceiling grid
