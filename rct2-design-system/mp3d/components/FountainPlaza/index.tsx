...
export function fountainPlazaPlan(input: FountainPlazaPlanInput): FountainPlazaPlan {
  ...
  if (tiles < 7) {
    console.info(`[FountainPlaza ${id}] tiles ${tiles} is too small for a walkable ring around the fountain — using 7`);
    tiles = 7;
  }
  if (tiles > 13) tiles = 13;
  ...
  SIDES.forEach((s, si) => {
    ...
    const name = open ? 'topiarySpiral' : input.planters === false ? 'topiarySpiral' : 'planterBox';
    [-1, 1].forEach((sgn, k) => {
    ...
    if (!open) {
      ...
      props.push({ at, name: 'marbleStatue', yaw: yawToward(at, base.position), scale: 1, seed: seed + si });
    }
  });
 ...
 */
export const FountainPlaza = setPiece<FountainPlazaPlan>('FountainPlaza', (t, plan, park) => {
  const g = new t.Group();
  ...
  // never one flat grey field) ----
  {
    ...
    acc.add(cyl(t, apron, apron, 0.02, 0xc6c5bc, [0, 0, 0], { tex: 'concrete', repeat: [8, 8], rough: 0.9, seg: 32 }));
    acc.add(cyl(t, apron - 0.32, apron - 0.32, 0.024, 0x8a8a83, [0, 0.002, 0], { tex: 'concrete', repeat: [6, 6], rough: 0.9, seg: 32 }));
  ...
  plan.spans.forEach(([a, b]) => {
    ...
    if (!ha || !hb) return;
    const span = setPieceSpan(t, ha, hb, { colors: [0xffd9a0, 0xffb46c, 0xfff0c8] });
    g.add(span.group);
    ups.push(span.update);
  });

  // topiary / planters / closing statues
